...
BinaryHeap.prototype.insert = function (value) {
  ...
  console.log("before swap",this._heap);
  let currIndex=this._heap.length-1;
  let pIndex = Math.floor((currIndex-1)/2);
  console.log(pIndex);
  while (this._heap[pIndex] && this._heap[currIndex] && this._heap[currIndex]<this._heap[pIndex]){
    this.swap(pIndex,currIndex);
    currIndex = pIndex;
    pIndex = Math.floor((currIndex-1)/2);
  }
  //return this._heap;
}
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  let smallerInd=null;
  if(this._heap[right]){
    smallerInd=(this._heap[left] < this._heap[right] ? left : right);
  }else{smallerInd=this._heap[left] ? this._heap[left] : null}
  while (this._heap[currIndex] && this._heap[smallerInd] && this._heap[currIndex]>this._heap[smallerInd]){
    this.swap(currIndex,smallerInd);
    currIndex=smallerInd;
    left = currIndex*2+1;
    right = currIndex*2+2;
    if(this._heap[right]){
      smallerInd=(this._heap[left] < this._heap[right] ? left : right);
    }else{smallerInd=this._heap[left] ? left : null}
  }
  return root;
}
...
console.log('before remove root:',bh._heap);
console.log(bh.removeRoot());
console.log('after remove root:',bh._heap);